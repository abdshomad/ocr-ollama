...
| Service | Role |
|---------|------|
| `vllm-deepseek` | DeepSeek-OCR (`:8100`, GPU `VLLM_DEEPSEEK_CUDA_DEVICE`) |
| `vllm-glm` | GLM-OCR (`:8101`, GPU `VLLM_GLM_CUDA_DEVICE`) |
| `nginx` | Serves UI at `/`, proxies `/api/` → backend |
| `backend` | FastAPI (internal port 8000, not published) |

**Requirements:** NVIDIA GPU + [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html). **Two GPUs recommended** (DeepSeek on GPU `0`, GLM on GPU `1`). After upgrading from the single-`vllm` layout, run `docker compose down --remove-orphans` so the old container does not hold a GPU. First `docker compose up` downloads weights into `vllm-hf-cache` (healthcheck allows up to ~30 min per model).
...
| `VLLM_DEEPSEEK_HOST` | `http://vllm-deepseek:8100` | DeepSeek-OCR vLLM |
| `VLLM_GLM_HOST` | `http://vllm-glm:8101` | GLM-OCR vLLM |
| `VLLM_DEEPSEEK_CUDA_DEVICE` | `1` | GPU for DeepSeek container |
| `VLLM_GLM_CUDA_DEVICE` | `0` | GPU for GLM container |
| `VLLM_GPU_MEMORY_UTILIZATION` | `0.85` | vLLM VRAM fraction per container |
...
**Dual vLLM OCR models:** Compose runs **`vllm-deepseek`** and **`vllm-glm`** in parallel (one GPU each by default). The Run and Arena pages list both `deepseek-ai/DeepSeek-OCR` and `zai-org/GLM-OCR` when their servers are healthy. See [issues/vllm-glm-ocr.md](issues/vllm-glm-ocr.md).
...
| Switch to GLM-OCR on vLLM | [issues/vllm-glm-ocr.md](issues/vllm-glm-ocr.md) |
