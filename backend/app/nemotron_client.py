...
from app.engine_registry import all_nemotron_models, feature_tags_from_ocr_engine, host_for_engine, model_entry


def format_nemotron_error(status_code: int, detail: Any, model: str) -> str:
    if isinstance(detail, dict):
        err_msg = str(detail.get("detail") or detail)
    else:
        err_msg = str(detail)
    return f"Nemotron OCR error ({status_code}) for model '{model}': {err_msg}"
    ...
    for ep, model_id in pairs:
        ...
        result.append(
            model_entry(
                ...
                engine_type=str(ep.get("type", "nemotron")),
...
async def ocr_chat(model: str, prompt: str, image_bytes: bytes) -> tuple[str, dict[str, Any], int]:
    ...
    if not ep:
        raise httpx.HTTPError(f"No Nemotron engine configured for model '{model}'")
    host = host_for_engine(ep)
    if not host:
        raise httpx.HTTPError(f"No Nemotron host configured for model '{model}'")
    ...
    data = {"prompt": prompt, "model": model, "merge_level": merge}
    ...
    async with httpx.AsyncClient(timeout=VLLM_TIMEOUT) as client:
        ...
        if r.status_code >= 400:
            ...
            raise httpx.HTTPStatusError(
                format_nemotron_error(r.status_code, detail, model),
                request=r.request,
                response=r,
            )
        body = r.json()
    ...
    meta = {
        "engine_type": "nemotron",
        "engine_label": str(ep.get("label") or "Nemotron OCR v2"),
        "nemotron_host": host,
        "duration_ms": body.get("duration_ms"),
        "merge_level": body.get("merge_level"),
    }
