...
from app.engine_registry import all_onnxtr_models, feature_tags_from_ocr_engine, host_for_engine, model_entry


def format_onnxtr_error(status_code: int, detail: Any, model: str) -> str:
    if isinstance(detail, dict):
        err_msg = str(detail.get("detail") or detail)
    else:
        err_msg = str(detail)
    return f"OnnxTR error ({status_code}) for model '{model}': {err_msg}"
...
async def list_models_with_classification() -> list[dict[str, Any]]:
    ...
    for ep, model_id in pairs:
        ...
        entry = model_entry(
            ...
            engine_type=str(ep.get("type", "onnxtr")),
...
async def ocr_chat(model: str, prompt: str, image_bytes: bytes) -> tuple[str, dict[str, Any], int]:
    ...
    if not ep:
        raise httpx.HTTPError(f"No OnnxTR engine configured for model '{model}'")
    host = host_for_engine(ep)
    if not host:
        raise httpx.HTTPError(f"No OnnxTR host configured for model '{model}'")
    ...
    async with httpx.AsyncClient(timeout=VLLM_TIMEOUT) as client:
        ...
        if r.status_code >= 400:
            ...
            raise httpx.HTTPStatusError(
                format_onnxtr_error(r.status_code, detail, model),
                request=r.request,
                response=r,
            )
        body = r.json()
    ...
    meta = {
        "engine_type": "onnxtr",
        "engine_label": str(ep.get("label") or "OnnxTR"),
        "onnxtr_host": host,
