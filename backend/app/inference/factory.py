...
from app import liteparse_client, mineru_client, nemotron_client, ollama_client, onnxtr_client, rapidocr_client, tesseract_client, vllm_client
from app.engine_registry import is_litparse_model, is_mineru_model, is_nemotron_model, is_onnxtr_model, is_rapidocr_model, is_tesseract_model
...
async def list_models_with_classification() -> list[dict[str, Any]]:
    ...
    if _backend() == "ollama":
        ...
        return [*ollama_models, *rapidocr_models, *onnxtr_models, *litparse_models, *tesseract_models]
    ...
    onnxtr_models = await onnxtr_client.list_models_with_classification()
    litparse_models = await liteparse_client.list_models_with_classification()
    return [*vllm_models, *mineru_models, *nemotron_models, *rapidocr_models, *onnxtr_models, *litparse_models, *tesseract_models]
...
async def check_health() -> dict[str, Any]:
    ...
    if _backend() == "ollama":
        ...
        if rapidocr_up or onnxtr_up or litparse_up or tesseract_up:
            raw["inference_reachable"] = True
            raw.pop("error", None)
        errs = [*rapidocr_errors, *onnxtr_errors, *litparse_errors, *tesseract_errors]
    ...
    if mineru_up or nemotron_up or rapidocr_up or onnxtr_up or litparse_up or tesseract_up:
        raw["inference_reachable"] = True
        raw["vllm_reachable"] = True
        raw.pop("error", None)
    errs = [*mineru_errors, *nemotron_errors, *rapidocr_errors, *onnxtr_errors, *litparse_errors, *tesseract_errors]
    if errs and not raw.get("inference_reachable"):
        raw["error"] = "; ".join(errs)
