...
from app.engine_registry import all_lanyocr_models, feature_tags_from_ocr_engine, host_for_engine, model_entry


def format_lanyocr_error(status_code: int, detail: Any, model: str) -> str:
    if isinstance(detail, dict):
        err_msg = str(detail.get("detail") or detail)
    else:
        err_msg = str(detail)
    return f"LanyOCR error ({status_code}) for model '{model}': {err_msg}"
...
async def list_models_with_classification() -> list[dict[str, Any]]:
    ...
    for ep, model_id in pairs:
        ...
        entry = model_entry(
            ...
            engine_type=str(ep.get("type", "lanyocr")),
...
async def ocr_chat(model: str, prompt: str, image_bytes: bytes) -> tuple[str, dict[str, Any], int]:
    ...
    if not ep:
        raise httpx.HTTPError(f"No LanyOCR engine configured for model '{model}'")
    host = host_for_engine(ep)
    if not host:
        raise httpx.HTTPError(f"No LanyOCR host configured for model '{model}'")
    ...
    async with httpx.AsyncClient(timeout=VLLM_TIMEOUT) as client:
        ...
        if r.status_code >= 400:
            ...
            raise httpx.HTTPStatusError(
                format_lanyocr_error(r.status_code, detail, model),
                request=r.request,
                response=r,
            )
        body = r.json()
    ...
    meta = {
        "engine_type": "lanyocr",
        "engine_label": str(ep.get("label") or "LanyOCR"),
        "lanyocr_host": host,
