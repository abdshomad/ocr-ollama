...
from app.engine_registry import all_rapidocr_models, feature_tags_from_ocr_engine, host_for_engine, model_entry


def format_rapidocr_error(status_code: int, detail: Any, model: str) -> str:
    if isinstance(detail, dict):
        err_msg = str(detail.get("detail") or detail)
    else:
        err_msg = str(detail)
    return f"RapidOCR error ({status_code}) for model '{model}': {err_msg}"
...
async def ocr_chat(model: str, prompt: str, image_bytes: bytes) -> tuple[str, dict[str, Any], int]:
    ...
    if not ep:
        raise httpx.HTTPError(f"No RapidOCR engine configured for model '{model}'")
    host = host_for_engine(ep)
    if not host:
        raise httpx.HTTPError(f"No RapidOCR host configured for model '{model}'")
    ...
    async with httpx.AsyncClient(timeout=VLLM_TIMEOUT) as client:
        ...
        if r.status_code >= 400:
            ...
            raise httpx.HTTPStatusError(
                format_rapidocr_error(r.status_code, detail, model),
                request=r.request,
                response=r,
            )
        body = r.json()
    ...
    meta = {
        ...
        "engine_label": str(ep.get("label") or "RapidOCR"),
        "rapidocr_host": host,
