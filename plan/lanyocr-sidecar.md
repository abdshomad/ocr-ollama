...
- **Image:** `docker/Dockerfile.cpu-ocr-sidecars` (target `lanyocr`) — `lanyocr` pulls `onnxruntime-gpu` on some platforms; **uninstall GPU wheel and install `onnxruntime` (CPU)** for a sane slim image.
